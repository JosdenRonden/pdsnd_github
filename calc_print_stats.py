...
def trip_duration_stats(df):
    ...
    trip_duration = []
    trip_duration.append(df['Trip Duration'].sum() / (60*60))
    trip_duration.append(df['Trip Duration'].mean() / (60))
    trip_duration.append(df['Trip Duration'].count()) 
    
    # string formatting
    # https://mkaz.blog/code/python-string-format-cookbook/
    print("Total travel time (hours):     {:.1f}".format(trip_duration  [0]))
    print("Average travel time (minutes): {:.1f}\n".format(trip_duration  [1]))
